...
CAreaFile* CAreaReader::read()
{
    ...
    ret->m_transform = CTransform(transformBasis, transformOrigin);
...
void CAreaReader::readModelHeader(CAreaFile* ret, atUint64& sectionStart, atUint32& i)
{
    ...
    {
        transformBasis[j].x = base::readFloat();
        transformBasis[j].y = base::readFloat();
        transformBasis[j].z = base::readFloat();
        transformOrigin[j] = base::readFloat();
    }
    model.m_transform = CTransform(transformBasis, transformOrigin);
    
    model.m_boundingBox.readBoundingBox(*this);
...
{
    atInt32 vertexCount = in.length() / 12;
...
void CAreaReader::readTexCoords(CModelData& model, Athena::io::MemoryReader& in, bool isLightmap)
{
    if (isLightmap)
    {
        atInt32 texCoordCount = in.length() / 4;
...
void CAreaReader::readMesh(CModelData& model, CAreaFile* ret, Athena::io::MemoryReader& in)
{
    ...
    if (ret->m_version != CAreaFile::DKCR)
    {
        ...
        if (extraDataSize > 0)
        {
            mesh.m_boundingBox.readBoundingBox(in);
        }
    }
    else
    {
